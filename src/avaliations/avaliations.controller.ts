...
export class AvaliationsController {
    ...
    @Get()
    findAll(
        @Query() query: FilterAvaliationDto,
    ): Promise<{ data: ResponseAvaliationDTO[]; total: number }> {
        return this.avaliationsService.findAll(query).then((res) => {
            return {
    ...
    findOne(@Param("hash") hash: string): Promise<ResponseAvaliationDTO> {
        return this.avaliationsService
            .findOne(hash)
    ...
    export(@Param("hash") hash: string): Promise<DisciplineFileData> {
        return this.avaliationsService.getDisciplineFile(hash).catch((err) => {
            console.log(err);
